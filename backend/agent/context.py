...
DEFAULT_DEVELOPER_INSTRUCTION = (
    ...
    "Create an evidence-driven multi-level folder/document tree as separate nodes first, then add optional sibling documents only when repository evidence supports them. Do not force every project into Backend and Frontend folders; create those folders only when the repository actually contains material backend or frontend code. For CLI tools, bots, libraries, worker-only services, infrastructure repositories, monorepos, mobile apps, and single-binary applications, organize documents around the real runtime, domain, storage, API, integration, deployment, and operational boundaries found in the snapshot. "
    "Each document must live under the most specific relevant folder, focus on one bounded subsystem or concern, and contain multiple reader-oriented sections instead of one omnibus report. Start with plain-language explanations of what the part is, why it exists, the mental model, and how a new reader should follow the flow; then provide evidence, bounded source excerpts, diagrams, formulas, risks, and verification steps. "
    "Do not leave key evidence as only a path or line range. When source evidence materially supports an important claim, introduce the file path and line range, then paste the smallest useful excerpt in a fenced code block with a language tag when known. "
    "Use portable Markdown only. Use block math with $$ on separate lines and inline math with $...$; do not use \\(...\\) or \\[...\\]. Mermaid diagrams must be valid and portable: use simple node IDs and quote labels that contain slashes, colons, parentheses, angle brackets, pipes, braces, file paths, or other punctuation. Do not use MDX-only components, custom HTML components, renderer-specific admonitions, malformed tables, or unterminated fences. "
    ...
    "Use concrete source evidence, bounded source excerpts, and web citations from tool results."
)
